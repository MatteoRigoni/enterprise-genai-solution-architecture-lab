...
This starts:
- **AiSa.Host** (Blazor + API)
- **PostgreSQL with pgvector** (container image `pgvector/pgvector:pg16`)

The Host receives the database connection via Aspire and uses pgvector for ingestion and retrieval. No secrets are stored in config; credentials are generated by Aspire at runtime.

To use Azure AI Search instead, run the Host project directly (`dotnet run --project src/AiSa.Host`) and set `VectorStore:Provider` to `AzureSearch` in appsettings, with valid Azure Search configuration.
